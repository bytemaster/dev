...
namespace json { namespace io {
    ...
    namespace detail {
      ...
      struct if_reflected {
        ...
        static inline void pack( json::value& s, const T& v ) { 
          ...
          // TO BASE 64
          json::io::pack(s,scrypt::base64_encode((unsigned char const*)ss.str().c_str(),ss.str().size()));
        }
        template<typename T>
        static inline void unpack( const json::value& s, T& v ) { 
          std::string str;
          json::io::unpack(s,str);
          std::stringstream ss(scrypt::base64_decode(str)); 
          ss >> v;
        }
      };
      template<>
    ...
    inline void pack( json::value& jsv, const std::vector<char>& data ) {
       if( data.size() ) { pack( jsv, scrypt::base64_encode((unsigned char*)&data.front(),data.size())); } 
    }
    inline void unpack( const json::value& jsv, std::vector<char>& data ) {
        data.clear();
        std::string d = scrypt::base64_decode(jsv);
